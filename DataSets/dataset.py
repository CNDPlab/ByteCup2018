...
class DataSet(Dataset):
    ...
    def __getitem__(self, item):
        ...
        return np.array(line['text_id']), np.array(line['summary_id'])
...
def own_collate_fn(batch):
    ...
    return t.LongTensor(text_id), t.LongTensor(title_id)


def get_dataloader(path, batch_size, num_worker):
    dataset = DataSet(path)
    dataloader = DataLoader(dataset, batch_size, shuffle=True, num_workers=num_worker, collate_fn=own_collate_fn, drop_last=True)
    return dataloader
